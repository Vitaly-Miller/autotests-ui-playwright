...
@pytest.fixture
def page(playwright: Playwright, storage_state: StorageState, request: SubRequest): # Используем фикстуру storage_state с авторизацией + встроенную фикстуру playwright из pytest_playwright plugin
    """
    Fixture with Storage state for authorized user (registered)
...
@pytest.fixture
def page_guest(playwright: Playwright, request: SubRequest):   # Чистый (без доп. фикстур)
    """
    Fixture for GUEST unregister user (NO Storage state)
